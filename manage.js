...
(async () => {
  ...
  if (command === 'add') {
    ...
    for (const tweetText of args) {
      await db.query('INSERT INTO posts (content) VALUES ($1)', [tweetText]);
      console.log(`✅ Added tweet: "${tweetText}"`);
    }
  }

  else if (command === 'list') {
    const { rows } = await db.query('SELECT id, content FROM posts ORDER BY id');
    if (!rows.length) {
    ...
    console.log(`📝 Posts left in this cycle: ${remaining < 0 ? 0 : remaining}`);
  }
